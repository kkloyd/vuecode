<template>
  <el-dialog :visible="visible" :title="title" :before-close="handleClose" custom-class="el-dialog--large">
...
export default {
  ...
  methods: {
    init(id) {
      let url = this.$route.meta.history
      this.title = this.$route.meta.historyTitle
      this.cols = []
      this.data = []
      this.loading = true
    ...
    exportTable() {
      ...
      http.downloadXLS(url + "Excel", body, "История").then(response => {
